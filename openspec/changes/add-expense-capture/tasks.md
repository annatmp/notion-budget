...
- [ ] 1.1 Initialise the Next.js (App Router, TypeScript) project with a Node runtime target and verify `npm run dev` serves a page and `npm run build` succeeds
- [ ] 1.2 Add a test runner and lint/typecheck scripts, and verify `npm test`, `npm run lint` and `npm run typecheck` all run clean on the empty project
- [ ] 1.3 Define the budget config schema (both data source IDs, property names, category list, currency options, default currency, timezone, model ID) with parse-time validation; verify unit tests cover a valid config, a config missing a required value, and a config with an unknown category
- [ ] 1.4 Load and validate config at startup so the app refuses to start on incomplete config with a message naming the missing value; verify by starting with a deliberately incomplete env and asserting the error names the field (`notion-integration` — "Budget binding is configuration")
- [ ] 1.5 Commit a `.env.example` documenting every variable, and confirm `.gitignore` excludes `.env*` so no credential can be committed
